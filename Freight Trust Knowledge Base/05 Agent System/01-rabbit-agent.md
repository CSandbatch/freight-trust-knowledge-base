...
`RESEARCH_PROGRAMME.md` → Open Research Questions).

## Output
Appends to `research/evidence.md`, one entry per source, using the shared evidence schema
in [ROSTER.md](ROSTER.md). Never resolves contradictions — two sources that disagree both
get entries.
