...
Load has locationofpickup, set to [place, position], named [current position].
Load has locationofloaddestination, which it needs to arrive to within [timewindow].
Trucker is approaching locationofpickup.
Trucker is returning to locationofloaddestination within [loadtimewindow].
...
Defining The Empty Mile Problem

The empty mile problem is composed of these things.
You have to solve these things to solve the empty mile problem:
Thing
Thing
thing

Solution Approach: An Open Knowledge Network for Cross-Actor Orchestration 
How things would have to move laterally  to produce orchestration.

	The map of the ecology
	With stations
	Where an activity is performed 

The Perfect Unit
A model of what happens when a mile is full.

Run of Show - Per Operator Type


On the ground
In the office 
Completing trips 
Managing risk, profit, and safety
...
And where there are integration points between things 

 
I was pleased to talk with all of you at the SC OKN workshop held at NIST on 9 and 10 March.  During the scenario subgroup of the Empty Mile breakout session, I mentioned a couple of related things that may help efforts to to demonstrate solutions to some of the pain points that seemed most promising to address with knowledge networks and related technology.  These were:
An industry association created OpenAPI for to support scheduling.  A web page through which one can gain access to this and other APIs supporting truck delivered goods is: https://dsdc.nmfta.org/apis . Other APIs currently available through this page include: electronic Bill of Lading (eBOL), pickup request and visibility, and preliminary freight charges.  These are surprisingly different from each other in style and format, and not as rich as I had hoped, but they still contain some relevant data types/elements and codes to the pain points we identified in the breakout.
The ASTM International Technical Committee F49 on Digital Information in the Supply Chain. A flyer, which is now slightly dated but still gives a good quick overview of the sort of work being performed in F49, is available here: https://mcsdocs.astm.org/committee-documents/F49%20Fact%20Sheet%202025.pdf .  A major output/area of work for this group is defining terms and data needed for the Goods Movement Process (GMP).  Major elements of this are definitions of events and statuses in the GMP (and codes for same). These are important types of data to capture and evaluate to reduce uncertainty around charges and costs including demurrage and detention charges. While movement of freight by truck is a small part of the scope of concern of the Goods Movement Process in F49, I have heard many of the key pain points discussed in our subbreakout also mentioned in F49 meetings, and there are experts involved in F49 who could be helpful in understanding these problems and how data could help reduce them, and arguments about them.
 
I also did a quick search for industry associations that would represent stakeholders in the truck shipping scenarios we discussed.  I am not an expert in this area, so I don’t know which of these associations would care or provide value for a proto-OKN pilot, nor do I know what organizations I may have missed in my search results.  But at least this list provides a start of some names and their links, that experts like Matt and Bola could look at to tell us which seem promising and what their concerns might be, and also identify those that are missing from the list.
...
Shippers: Food Shippers of America (FSA)
