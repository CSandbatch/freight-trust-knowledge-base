...
""",
            ...
            self.assertIn("Private agent session", project)
            self.assertNotIn("Freight runs on fragmented trust.", project)
            self.assertIn('class="home-page"', project)
            ...
            for index in range(1, 6):
                ...
                self.assertIn("Research logic", experiment)
                self.assertIn("Decision enabled", experiment)
                self.assertIn("If evidence is null or negative", experiment)
                self.assertIn("Inputs and evidence", experiment)
                self.assertIn("Execution path", experiment)
                self.assertIn("Validity, governance, and reproducibility", experiment)
                self.assertIn("Claim boundary", experiment)
                self.assertIn("Complete experiment protocol", experiment)
                self.assertIn(f"Fixture thesis for E{index}.", experiment)
