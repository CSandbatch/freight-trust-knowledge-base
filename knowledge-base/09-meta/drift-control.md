...
|---|---|---|---|---|
| ~~`DRIFT-003`~~ | D3 | [[kb-schema]], [[experiment-protocol-standard]], [[datasets-and-experiments-moc]] | ~~Programme decision gates reused the protocol `G0`–`G5` namespace.~~ | **Fixed 2026-08-18** — programme gates now use `DG-P0` and `DG-E1`–`DG-E5`; research goals and legacy protocol quality gates remain qualified. |
| ~~`DRIFT-004`~~ | D1 | [[experiment-e5-orchestration-value]] vs [[datasets-and-experiments-moc]] | ~~E5's execution trigger and result gate were circular.~~ | **Fixed 2026-08-18** — Phase I specifies and smoke-tests E5; full execution is Phase II by default, and its result feeds `DG-E5`. |
...
| ~~`DRIFT-011`~~ | D6 | [[experiment-e4-participation-and-small-carrier-equity]] | ~~The burden claim was unhedged in one place and hedged in another.~~ | **Fixed 2026-08-18** — both descriptions now state that organization size and staffing may alter burden; E4 measures rather than assumes the effect. |
| ~~`DRIFT-012`~~ | D1 | [[experiment-e2-facility-event-provenance-and-dwell-reconstruction]] | ~~The thesis event list included loading but the interval model did not.~~ | **Fixed 2026-08-18** — the thesis now names the same appointment, arrival, gate, dock, release and departure milestones used by the interval model. |
...
| ~~`DRIFT-015`~~ | D5 | [[experiment-e3-federated-access-and-policy-enforcement]] | ~~An unverified NIST DP challenge asset was used as design support.~~ | **Fixed 2026-08-18** — the E3 protocol now uses an authority-bearing project oracle and pinned native engine lanes; the unsupported reliance was removed. |
| ~~`DRIFT-021`~~ | D1 | [[04-sbir/drafts/data-management-plan-draft]] | ~~A security bullet asked for provider and rate.~~ | **Fixed 2026-08-18** — the DMP now requires provider plus control implementation and verification evidence. |
...
| `DRIFT-029` | Med | D6 | retired Publishing Agent specification (now root `publisher`) | The spec requires citations for "$15B detention", canonicalizing a figure that [[sbir-evidence-refresh]] flags as possible stale-figure risk pending primary-PDF confirmation. The client briefs use the $3.6B / $11.5B breakdown instead | Remove the figure from the spec, or point it at the refresh's caveat | Publishing |
| `DRIFT-030` | Med | D4 | retired Visualization Agent specification (now root `visualization_engineer`) | Cites `AGENT_framework_plan.md` Modules 4–5 as the source for the stakeholder matrix. That file was deleted in the current staged changeset; the reference now points at nothing on disk | Repoint to the surviving source, or record the source as retired | Agent-system owner |
...
picking a side. `DRIFT-038` and `DRIFT-039` were later closed by direct retrieval and
predicate-specific correction on 2026-08-20; the historical findings remain below.

| ID | Sev | Class | Where | Issue | Proposed fix | Owner |
|---|---|---|---|---|---|---|
| ~~`DRIFT-038`~~ | Med | D6 | [[dataset-scan-entity-resolution]] footer vs. [[dataset-scan-event-provenance-and-federation]] Aim 2 table | **Resolved 2026-08-20.** The FMCSA Open Data Program page was retrieved directly and reconciled into the source card, scan, and dataset index. It confirms named file families and stated cadences but does not supply a blanket redistribution licence | Closure evidence: [[source-fmcsa-mcmis-catalog]] and [[run-20260820-002-e1-stage-0-4]] | Research lead |
| ~~`DRIFT-039`~~ | Med | D6 | [[dataset-fmca-registration-insurance-safety-records]] vs. [[dataset-scan-entity-resolution]] | **Resolved 2026-08-20.** Direct metadata shows `n46a-mfgy` is a public legacy `href` stub with zero columns, while named Motus assets are the current baseline/difference files. The dataset card now uses predicate-specific access wording and keeps licence, schema-cutover, and safety-family limits open | Closure evidence: [[source-fmcsa-licensing-and-insurance-dataset]], [[dataset-fmca-registration-insurance-safety-records]], and [[run-20260820-002-e1-stage-0-4]] | Research lead |
