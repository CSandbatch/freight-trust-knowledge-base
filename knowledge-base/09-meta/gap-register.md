...
|---|---|---|---|---|
| `GAP-001` | **Applicant legal form, route, and PI eligibility documentation remain unresolved** — BellHill is the intended applicant and Russell Berry is the working PI under [[09-meta/decisions/dec-014-bellhill-applicant-and-pi]] | Working names are settled, but SBIR/STTR eligibility still depends on BellHill's exact registered name, legal form, ownership/affiliates and Russell Berry's employment, effort, work-authorization and research-security facts | Registered name/entity type and ownership corroborated; SBIR vs. STTR route chosen; Russell Berry's majority-employment, effort and other PI eligibility evidence documented | BellHill + Russell Berry + counsel (`DEC-014`) |
| `GAP-002` | **Some legacy source failures remain unresolved; FMCSA Census/Open Data/Motus portions are closed** | Direct retrieval now covers the FMCSA Open Data Program, Company Census metadata/schema/dictionary/export, the legacy non-tabular L&I stub, and named Motus successor families. Remaining unresolved portions are PIERS licence terms, OpenCorporates pricing/terms, and any statutory or source card still marked retrieval-failed; do not carry the FMCSA resolution across to those sources | Each remaining source retrieved by a method that works, with `accessed` and `verification: confirmed`, or reclassified as permanently unavailable with the failure documented | `source-scout` (L2) |
| `GAP-003` | **E1 identity/adjudication standard is RC1-complete; human freeze and E2-specific rubric remain open** | Research/eval produced [[e1-carrier-identity-and-relationship-standard]], ontology, decision tree, 64-claim ledger, 70-case suite, hostile review, [[e1-adjudicator-governance-and-training]], [[e1-definition-conformance-report]], and [[e1-state-corporate-source-access-memo]]. E1 no longer lacks a rubric, written COI/training protocol, or initial state-source workflow. Remaining E1 gates are PI/domain/counsel approval, actual reviewer training, pilot double-labeling, and case-level source retrieval. E2 still needs its own facility-event adjudication rubric | E1 RC1 approved/frozen and pilot double-labeling run completed; E2 rubric separately written before E2 gold labeling | Russell Berry + technical lead + domain reviewer + counsel |
