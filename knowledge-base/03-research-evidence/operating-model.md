...
reached; record `unverified`, `contradicted`, or `blocked`, and never silently upgrade
confidence.
...
Maintain [[run-log]] as the append-only observability record. Each row should capture:
...
1. Add stable claim IDs (`G1-C01`, etc.) to `03-research-evidence/evidence.md`.
2. Maintain [[run-log]] and create one task packet per open goal in [[plan]].
