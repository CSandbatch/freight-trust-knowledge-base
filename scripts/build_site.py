...
EXPERIMENT_PROGRAM = (
    {
        ...
        "decision": "E1 informs whether Freight Trust should advance an identity-resolution capability into a bounded carrier-onboarding workflow, narrow the task to evidence assembly for human review, or stop because the available anchors cannot support a sufficiently safe partition of legal persons.",
        "scope": "The unit of analysis is an entity-centric case containing a candidate set of temporally bounded observations. Task A asks whether observations belong to the same legal person. Task B records registrant continuity and other relationships separately. Task C preserves regulatory disposition and procedural status. The first benchmark is deliberately bounded by jurisdiction, source snapshot, cutoff date, and adjudication policy.",
        "support": "A positive result would show that the frozen resolver improves safe yield at the declared false-positive ceiling on the adjudicated benchmark, remains calibrated enough to abstain on unsupported cases, and does not hide failures in sparse or difficult subgroups. It would justify the next bounded validation step, not production autonomy.",
        "null": "A null or negative result would mean that the tested resolver does not improve the safety-yield tradeoff over its transparent baselines under the frozen benchmark. The reference corpus, ontology, source adapters, and adjudication workflow may still be valuable deliverables. The programme would narrow or stop the resolver claim rather than reinterpret the benchmark after seeing results.",
        "validity": "Major threats include label error, duplicated anchors, source-specific shortcuts, temporal leakage, benchmark closure mistakes, non-independent cases, changing agency guidance, and a benchmark that is easier than the intended workflow. The design therefore separates development and holdout work, simulates duplicated-anchor bias, tests chronology, and reports sensitivity to adjudication uncertainty.",
        "governance": "Identity errors can exclude legitimate carriers or merge unrelated businesses. The experiment requires abstention, evidence display, correction and appeal, separate relationship types, reviewer accountability, and a deny-by-default holdout boundary. Public records are not automatically unrestricted for redistribution, and official identifiers are not universal legal-person gold.",
        "reproducibility": "A complete E1 run packet must pin schemas, source snapshot identifiers and hashes, field maps, rights classifications, candidate rules, C1 rule table, C2 parameters, code revision, seeds, split manifests, predictions, adjudication version, estimators, intervals, deviations, and the final claim language. The held-out evaluation is opened once and preserved unchanged.",
    ...
    },
    {
        ...
        "decision": "E2 informs whether the event-evidence architecture can produce a defensible, uncertainty-aware timeline from fragmented observations and whether a permissioned facility validation is warranted. It also determines which anomaly families are detectable and which must remain unresolved.",
        "scope": "The initial unit is a generated shipment or facility trace with hidden canonical events and a separately defined observability process. The freight profile is expressed through EPCIS documents plus namespaced extensions. Synthetic development and benchmark claims remain separate from any later permissioned partner holdout, reviewer study, or real dispute workflow.",
        "support": "A positive synthetic result would show that the pinned reconstruction conditions reduce declared timeline or dwell error, preserve censoring when endpoints are absent, and identify specified alteration, omission, or contradiction operators with acceptable uncertainty and review burden. It would support architecture feasibility, not real-facility accuracy.",
        "null": "A null result would mean that provenance-aware reconstruction does not improve the frozen error, sufficiency, or review-time outcomes in the declared traces. It could reveal that the observation model is underdetermined, source policy is doing the work, or uncertainty intervals are too wide to support the intended decision. Those are useful design findings and must not be repaired by exposing hidden truth to the system under test.",
        "validity": "Threats include an unrealistic generator, leakage of hidden labels, source-priority tuning on benchmark truth, incomparable clock models, outcome switching between dwell and adjudication, and treating injected anomalies as evidence of malicious tampering. External validity remains untested until a permissioned holdout uses independently observed facility data.",
        "governance": "The benchmark must physically isolate hidden truth from the reconstructor, minimize commercially sensitive fields, label inferred events, preserve corrections, and distinguish content anomalies from cryptographic integrity. Reviewer work requires the applicable institutional determination, and partner data requires an explicit access, retention, and publication agreement.",
        "reproducibility": "A complete E2 packet pins the EPCIS and CBV artefacts, freight namespace, generator release or commit, schema and SHACL versions, trace seed, observability model, anomaly manifest, source policy, condition implementation, hidden-truth isolation test, evaluation code, intervals, privacy result, and hashes for every generated and reported artifact.",
    ...
    },
    {
        ...
        "decision": "E3 informs whether a specific policy stack is sufficiently authenticated, enforceable, auditable, and correctable to enter a bounded E1-plus-E3 onboarding integration. A failure can stop the integration even when the underlying identity evidence is accurate.",
        "scope": "The unit is a versioned request case evaluated against an authority-approved domain expectation and one pinned engine-native policy lane. NGAC and XACML remain separate experiments when both are used. Native engine responses are preserved before mapping to the domain decision, and policy enforcement point behavior is part of the tested system.",
        "support": "A positive result would establish measured conformance to the frozen oracle and suite: valid requests receive the expected domain decision, invalid identities fail closed, obligations are actually enforced, every attempt reconciles to the independent ledger, mutations are detected, and correction retains superseded history. It supports only the tested configuration.",
        "null": "A negative result identifies a mismatch among oracle, policy, adapter, engine, identity configuration, enforcement point, or audit model. The correct response is to localize and repair the failing layer, rerun qualification fixtures, and preserve the failure record. It is not evidence that policy-governed federation is impossible in general.",
        "validity": "Threats include testing only happy paths, using default or sandbox authentication, silently translating native policies, trusting issuer-signed claims as independent organizational truth, omitting obligation enforcement, losing rejected attempts, treating a hash chain as capture authenticity, and pooling incomparable engine lanes.",
        "governance": "The policy authority and reviewer must be named before the oracle is treated as legitimate. JWT algorithms, issuer, audience, expiry, subject binding, key rotation, and token type are mandatory test dimensions. The public corpus contains fixtures and bounded policies only; secrets, production identities, private authorization data, and operational audit streams remain outside it.",
        "reproducibility": "A complete E3 packet pins the policy corpus, authority review, engine and adapter commits, container or executable hashes, runtime, JDK, JWT/JWKS fixtures, issuer and audience configuration, native request and response, domain mapping, PEP outcomes, independent ledger, canonical JSON bytes, chain checkpoints, mutation tests, correction results, and all deviations.",
    ...
    },
    {
        ...
        "decision": "E4 informs whether a concrete participation offer is understandable, usable, proportionate, and sufficiently promising to justify a powered field study. It can also stop or redesign the programme when burden, exclusion, refusal, or correction failures exceed frozen gates.",
        "scope": "Before documented institutional or sponsor determination, the unit is a synthetic fixture or non-interactive instrument artifact. An authorized pilot would define the recruited organization or user, assignment unit, cluster, exposure mapping, fleet-size band, offer condition, and observation window. Recruited participants are not assumed to represent the freight sector.",
        "support": "A positive feasibility result would show that the offer can be delivered as described, participants understand the information and correction process, observed burden remains within the frozen threshold, and activation or repeat use is estimable without unacceptable subgroup deterioration. It would justify a larger study, not an adoption claim.",
        "null": "A null result may mean no detectable offer effect under the frozen exposure mapping. A design failure is different: insufficient clusters, contamination, unusable instruments, undeliverable benefits, sparse cells, or a mapping that cannot identify the estimand. The report must distinguish evidence against the offer from evidence that the study could not answer the question.",
        "validity": "Threats include self-selection, spillover between participants, organizational clustering, social-desirability responses, mock offers mistaken for real behavior, attrition, burden underreporting, inaccessible instruments, sparse small-carrier cells, and overgeneralization from a recruited frame. Power and disclosure rules must be frozen before contact.",
        "governance": "No recruitment, outreach, pilot interaction, or participant-level collection begins without the responsible institutional process. Every arm receives the same approved information and consent process, including any approved waiver or alteration. Identifiable records stay in a private store; public artifacts are blank instruments, schemas, code, synthetic fixtures, and disclosure-controlled aggregates.",
        "reproducibility": "A complete E4 packet pins the determination or approval, recruitment frame, instrument versions and checksums, accessibility checks, assignment code and seed, cluster and exposure map, offer implementation, burden definitions, primary estimand, MDE assumptions, thresholds, sparse-cell rules, attrition handling, analysis code, disclosure review, deviations, and aggregate outputs.",
    ...
    },
    {
        ...
        "decision": "E5 informs whether a governed planning concept is technically and scientifically promising enough for Phase II experimentation. It asks whether better information changes a bounded decision after hard feasibility and actor-protection gates, not whether a full freight network should deploy an optimizer.",
        "scope": "The unit is a versioned synthetic scenario drawn from a declared operating population and policy environment. The first HOS scope must state vehicle and operation applicability and enumerate every included exception. Solomon instances validate a routing-with-time-windows convention only; BTS observations can calibrate selected travel-time priors but not facility dwell or nationwide operating realism.",
        "support": "A positive result would show improvement on one frozen primary planning outcome against executable local baselines under identical information rules, while every accepted route passes the independent HOS checker and service, safety, margin, burden, and actor-level deterioration remain within their gates. It would justify a more realistic validation programme.",
        "null": "A null result means governed information does not improve the primary outcome in the tested scenarios and policies. It does not prove that empty miles are unrelated to dwell or that all coordination is valueless. Infeasible alternatives, solver timeouts, and constraint violations are diagnostics or stop conditions, not outcomes that can be averaged into apparent performance.",
        "validity": "Threats include outcome switching, optimistic synthetic priors, solver advantage unrelated to information, inconsistent baseline information sets, invalid HOS shortcuts, facility-dwell calibration from travel-time data, weight-sensitive aggregate gains, hidden actor losses, and scenarios too simple to expose operational conflicts. Ablations and stress tests are required.",
        "governance": "Hard safety and HOS rules are constraints, never negotiable objectives. Actor-level veto or non-inferiority rules must be frozen before evaluation. Upstream E1-E4 outputs enter only after acceptance and with their uncertainty and rights attached. No synthetic result authorizes dispatch, pricing, employment, access, or other operational decisions.",
        "reproducibility": "A complete E5 packet pins the scenario schema, operating population, HOS state machine and test vectors, solver and runtime hashes, P0/P1/P3/P4 algorithms, information sets, tie-breaks, timeouts, infeasibility handling, seeds, common random numbers, calibration extracts and checksums, priors, thresholds, actor distributions, ablations, sensitivities, Pareto sets, and failure logs.",
...
def render_markdown(artifact: Artifact, by_source: dict[str, Artifact], render_page: str | None = None) -> str:
    ...
    def replace_wikilink(match: re.Match[str]) -> str:
        ...
        href = page_href(render_page or artifact.page, resolved.url)
        if anchor:
            href += f"#{quote(anchor)}"
...
<body class="{html.escape(active, quote=True)}-page" data-root="{html.escape(data_root, quote=True)}" data-search="{html.escape(page_href(page, 'data/search.json'), quote=True)}" data-catalog="{html.escape(page_href(page, 'data/catalog.json'), quote=True)}" data-chat-endpoint="/api/chat">
...
def home_page(page: str, artifacts: list[Artifact], links: list[Link], site_url: str) -> str:
    ...
    main = f'''<section class="project-hero harness-hero" data-agent-hero>
  <div class="harness-shell">
    <aside class="harness-rail" aria-label="Hermes agent details">
      <div class="rail-brand"><span class="rail-mark" aria-hidden="true">FT</span><span>FREIGHT TRUST<small>PRIVATE KNOWLEDGE SYSTEM</small></span></div>
      <div class="rail-rule"></div>
      <p class="rail-kicker">Hermes / 01</p>
      <p class="rail-title">A research companion for the working record.</p>
      <dl class="rail-stats"><div><dt>Corpus</dt><dd>{len(notes)} notes</dd></div><div><dt>Links</dt><dd>{len(links):,} resolved</dd></div><div><dt>State</dt><dd><span class="status-dot"></span>private</dd></div></dl>
      <a class="rail-browse" data-show-browser href="{html.escape(page_href(page, 'knowledge/index.html'), quote=True)}">Browse the knowledge base <span aria-hidden="true">↗</span></a>
      <p class="rail-foot">Grounded in the published working set.<br>Verify consequential claims in source notes.</p>
    </aside>
    <div class="harness-main">
      <header class="harness-topline"><div><p class="eyebrow">Private agent session</p><h1>Ask Hermes.</h1></div><div class="session-indicator"><span class="status-dot"></span><span>Private session</span><button class="session-login" type="button" data-login-trigger>Log in</button><code>GLM-5.3 / HERMES</code></div></header>
      <section class="agent-chat" data-agent-chat aria-label="Chat with the private Hermes knowledge agent"><div class="chat-transcript" data-chat-transcript aria-live="polite"><article class="chat-message is-assistant"><span>Hermes · Knowledge agent</span><p>What would you like to understand about Freight Trust?</p><small>Answers are drawn from the linked research record, with source notes when available.</small></article></div><form class="chat-composer" data-chat-form><label class="sr-only" for="agent-chat-input">Ask Hermes about the knowledge base</label><textarea id="agent-chat-input" data-chat-input rows="2" maxlength="8000" placeholder="Ask about evidence, experiments, open decisions, or claim boundaries…" required></textarea><div class="chat-composer-actions"><label class="chat-image-button" title="Attach a PNG, JPEG, or WebP image"><input data-chat-image type="file" accept="image/png,image/jpeg,image/webp"><span aria-hidden="true">Attach</span><span class="sr-only">Attach image</span></label><span class="chat-attachment" data-chat-attachment></span><span class="composer-hint">Enter to send · Shift + Enter for a new line</span><button class="chat-send" data-chat-send type="submit">Send</button></div></form><p class="chat-status" data-chat-status>Sign in to enable the private Obsidian and research agent.</p></section>
      <p class="harness-boundary">Private working session · Not legal, regulatory, or operational advice.</p>
    </div>
  </div>
</section>
...
<section class="section-shell publication-note"><div><p class="eyebrow">Current state</p><h2>Build-start-ready. Scientifically unrun.</h2></div><div><p>Protocol and fixture implementation may begin. Benchmark, partner, protected-data, human-subjects, and confirmatory gates remain closed until their named approvals and numeric locks are recorded.</p><a href="{html.escape(programme_href, quote=True)}">See the exact programme boundaries</a></div></section>'''
    main = main.split('<section class="section-shell whitepaper-section problem-section">', 1)[0]
    return shell_html("Freight Trust Infrastructure", "A private Hermes research harness over the Freight Trust knowledge base.", page, site_url, main, "home")
...
def experiment_page(config: dict[str, str], protocol: Artifact, artifacts: list[Artifact], page: str, site_url: str) -> str:
    ...
    outcome = str(protocol.metadata.get("primary_outcome", "not frozen")).replace("-", " ")
    protocol_href = page_href(page, protocol.url)
    protocol_raw_href = page_href(page, protocol.raw)
    protocol_body = render_markdown(protocol, by_source, page)
    graph_href = page_href(page, f"graph/index.html?focus={quote(protocol.source)}&mode=local&depth=1")
...
<header class="experiment-page-hero"><div class="experiment-page-code"><span>{config['id']}</span><small>{html.escape(config['role'])}</small></div><div><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="{html.escape(page_href(page, 'experiments/index.html'), quote=True)}">Experiments</a><span>/</span><span>{config['id']}</span></nav><p class="eyebrow">Experiment white paper / {html.escape(config['phase'])} / Build-start-ready</p><h1>{html.escape(config['title'])}</h1><p class="experiment-page-question">{html.escape(config['question'])}</p><div class="experiment-actions"><a class="button" href="#protocol-appendix">Read the full protocol below</a><a class="button button-quiet" href="{html.escape(graph_href, quote=True)}">Trace the evidence</a><a class="button button-quiet" href="{html.escape(overview_href, quote=True)}">Programme overview</a></div></div></header>
<nav class="experiment-paper-toc" aria-label="In this experiment white paper"><p class="eyebrow">In this paper</p><ol><li><a href="#executive-summary">Executive summary</a></li><li><a href="#research-logic">Research logic</a></li><li><a href="#study-design">Study design</a></li><li><a href="#execution-path">Execution path</a></li><li><a href="#programme-interface">Programme interface</a></li><li><a href="#validity-governance">Validity and governance</a></li><li><a href="#claim-boundary">Claim boundary</a></li><li><a href="#protocol-appendix">Full protocol</a></li><li><a href="#source-records">Source records</a></li></ol></nav>
<section class="experiment-page-section experiment-explainer" id="executive-summary"><div><p class="eyebrow">Executive summary / why this experiment exists</p><h2>{html.escape(config['problem'])}</h2></div><div><p class="eyebrow">Working hypothesis</p><p>{html.escape(config['hypothesis'])}</p><p class="experiment-state-note"><strong>Current state:</strong> the protocol is ready for implementation work, but no scientific run or finding exists.</p></div></section>
<section class="experiment-page-section experiment-logic" id="research-logic"><div class="section-heading"><p class="eyebrow">Research logic</p><h2>The decision this experiment is designed to inform</h2><p>A research result matters only when its intended decision, population, and interpretation are fixed before the outcome is known.</p></div><div class="experiment-logic-grid"><div><span>Decision enabled</span><h3>Advance, narrow, redesign, or stop</h3><p>{html.escape(config['decision'])}</p></div><div><span>Scope and unit</span><h3>What the result actually describes</h3><p>{html.escape(config['scope'])}</p></div><div><span>If evidence is positive</span><h3>What support would be earned</h3><p>{html.escape(config['support'])}</p></div><div><span>If evidence is null or negative</span><h3>What failure would mean</h3><p>{html.escape(config['null'])}</p></div></div></section>
<section class="experiment-page-section" id="study-design"><div class="section-heading"><p class="eyebrow">Study design</p><h2>What goes in, what is compared, and what is measured</h2><p>The design keeps evidence inputs, experimental comparisons, and evaluation measures separate so a strong result cannot be manufactured by changing the task after implementation.</p></div><div class="experiment-design-grid"><div><span>01</span><h3>Inputs and evidence</h3><p>{html.escape(config['inputs'])}</p></div><div><span>02</span><h3>Comparison</h3><p>{html.escape(config['comparison'])}</p></div><div><span>03</span><h3>Measures</h3><p>{html.escape(config['measures'])}</p></div></div></section>
<section class="experiment-page-section experiment-execution" id="execution-path"><div><p class="eyebrow">Execution path</p><h2>Build first. Lock decisions before the real run.</h2><p class="muted">Implementation work can begin against public or synthetic fixtures. Opening a protected benchmark, recruiting participants, or making a confirmatory claim requires the separate gates described below.</p></div><ol><li><span>1</span><div><strong>Freeze scope and semantics</strong><p>{html.escape(config['depends'])}</p></div></li><li><span>2</span><div><strong>Implement the first build slice</strong><p>{html.escape(config['build'])}</p></div></li><li><span>3</span><div><strong>Pass fixture and conformance checks</strong><p>Run deterministic smoke cases, negative tests, leakage checks, and packet validation without opening protected or held-out data.</p></div></li><li><span>4</span><div><strong>Close the real-run gate</strong><p>{html.escape(config['gate'])}</p></div></li><li><span>5</span><div><strong>Run once and report at the earned level</strong><p>Package pinned inputs, code, configuration, outputs, failures, deviations, and limits in an immutable run record.</p></div></li></ol></section>
<section class="experiment-page-section experiment-contract" id="programme-interface"><div><p class="eyebrow">Programme interface</p><h2>What this experiment contributes</h2><p>{html.escape(config['produces'])}</p><p class="muted">Downstream experiments consume only the versioned output contract and retain its uncertainty, rights, correction state, and prohibited inferences.</p></div><dl><div><dt>Status</dt><dd>{html.escape(protocol.status)}</dd></div><div><dt>Phase</dt><dd>{html.escape(phase)}</dd></div><div><dt>Owner</dt><dd>{html.escape(owner)}</dd></div><div><dt>Primary outcome</dt><dd>{html.escape(outcome)}</dd></div></dl></section>
<section class="experiment-page-section experiment-assurance" id="validity-governance"><div class="section-heading"><p class="eyebrow">Validity, governance, and reproducibility</p><h2>How the experiment protects the meaning of its result</h2><p>A technically successful run is not credible when the benchmark leaks, the tested population is unclear, participant protections are missing, or another team cannot reconstruct the packet.</p></div><div class="experiment-assurance-grid"><div><span>Validity threats</span><p>{html.escape(config['validity'])}</p></div><div><span>Governance and safeguards</span><p>{html.escape(config['governance'])}</p></div><div><span>Reproducibility packet</span><p>{html.escape(config['reproducibility'])}</p></div></div></section>
<section class="experiment-claim-boundary" id="claim-boundary"><div><p class="eyebrow">Claim boundary</p><h2>A positive result would still stop here.</h2><p>{html.escape(config['boundary'])}</p></div></section>
<section class="protocol-appendix" id="protocol-appendix"><header><div><p class="eyebrow">Authoritative appendix</p><h2>Complete experiment protocol</h2><p>The explanatory paper above is an orientation layer. This rendered, version-controlled protocol remains authoritative for the research design, definitions, estimands, conditions, gates, and reporting requirements.</p></div><dl><div><dt>Source</dt><dd>{html.escape(protocol.source)}</dd></div><div><dt>Status</dt><dd>{html.escape(protocol.status)}</dd></div><div><dt>SHA-256</dt><dd>{html.escape(protocol.sha256)}</dd></div></dl><div class="protocol-appendix-actions"><a class="button" href="{html.escape(protocol_href, quote=True)}">Open protocol reader</a><a class="button button-quiet" href="{html.escape(protocol_raw_href, quote=True)}" download>Download exact source</a></div></header><div class="protocol-paper reader-content">{protocol_body}</div></section>
<section class="experiment-page-section experiment-records" id="source-records"><div class="section-heading"><p class="eyebrow">Source layer</p><h2>Protocol and directly linked records</h2><p>These are the authored records that define or support this experiment. Their status, confidence, source class, and limitations remain controlling.</p></div><div class="experiment-record-grid"><a href="{html.escape(protocol_href, quote=True)}"><span>Protocol</span><strong>{html.escape(protocol.title)}</strong><small>{html.escape(article_excerpt(protocol, 150))}</small></a>{support_html}</div></section>
