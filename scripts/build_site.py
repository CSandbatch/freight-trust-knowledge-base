...
def main() -> None:
    ...
    note_paths = {note["path"]: note["path"] for note in notes}
    note_paths.update({note["path"][:-3]: note["path"] for note in notes})
    note_names: dict[str, list[str]] = {}
    for note in notes:
        note_names.setdefault(pathlib.PurePosixPath(note["path"]).stem, []).append(note["path"])
    ...
    for note in notes:
        targets = re.findall(r"\[\[([^\]|]+)(?:\|[^\]]*)?\]\]", note["text"])
        note["links"] = sorted({resolved for target in targets if (resolved := resolve_link(note["path"], target))})
...
function render(){let found=notes.filter(matches),visible=new Set(found.map(x=>x.path)),pos=new Map;count.textContent=found.length+' documents';r.innerHTML='';found.slice(0,45).forEach(x=>{let b=document.createElement('button');b.className='result';b.innerHTML=esc(x.title)+'<small>'+esc(x.section+' · '+x.path)+'</small>';b.onclick=()=>show(x);r.append(b)});map.innerHTML='';found.forEach((x,i)=>{let ring=.12+(i%23)/29,angle=(i*137.508+notes.findIndex(n=>n.path===x.path)*11)*Math.PI/180;pos.set(x.path,[50+Math.cos(angle)*ring*92,50+Math.sin(angle)*ring*92])});let svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.classList.add('edges');found.forEach(x=>x.links.filter(p=>visible.has(p)&&x.path<p).forEach(p=>{let a=pos.get(x.path),b=pos.get(p),l=document.createElementNS(svg.namespaceURI,'line');l.setAttribute('x1',a[0]+'%');l.setAttribute('y1',a[1]+'%');l.setAttribute('x2',b[0]+'%');l.setAttribute('y2',b[1]+'%');svg.append(l)}));map.append(svg);found.forEach(x=>{let p=pos.get(x.path),b=document.createElement('button');b.className='dot'+(selected===x.path?' selected':'');b.style.left=p[0]+'%';b.style.top=p[1]+'%';b.title=x.title;b.setAttribute('aria-label',x.title);b.onclick=()=>show(x);map.append(b)})}
