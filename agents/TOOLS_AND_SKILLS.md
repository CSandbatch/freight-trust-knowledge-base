...
human operator can maintain [`research/run-log.md`](../research/run-log.md), create task
...
Recommended future capabilities are: stable claim IDs, source retrieval snapshots,
...
The current capability contract is [MCP_CAPABILITIES.md](MCP_CAPABILITIES.md). The
project-local reusable workflow is
[`freight-trust-research`](skills/freight-trust-research/SKILL.md).

What each agent in [ROSTER.md](ROSTER.md) actually needs to do its job — split into what's
...
(Phase 1 of [research/PLAN.md](../research/PLAN.md)), so this reflects real friction, not
guesswork.
...
- Google Drive search (`mcp__claude_ai_Google_Drive__*`) — the team's notes reference
  ...
  have yet, before assuming `raw/` is the complete team record.
- PDF/DOCX-to-markdown extraction (the small `pypdf`/zip-XML scripts used to build `raw/`)
...
needed beyond what's already in use for `RESEARCH_PROGRAMME.md`.
...
already spec'd in [03-review-agent.md](03-review-agent.md).
...
specific query attached, rather than just flagging it as unsupported.
...
**Available now:**
- Mermaid diagrams (already used for the pipeline diagram in `ROSTER.md`) — sufficient
  for the duty-of-care chain, OKN architecture, and process-flow diagrams in
  `research/PLAN.md` Phase 7.
